...
class ContextUnet(nn.Module):
    def __init__(self, in_channels, n_feat = 256, n_classes=10, n_domains=4):
        ...
        self.contextembed_domain1 = EmbedFC(n_domains, 2*n_feat)
        self.contextembed_domain2 = EmbedFC(n_domains, 1*n_feat)
    ...
    def forward(self, x, c, domain, t, context_mask):
        ...
        domain = nn.functional.one_hot(domain, num_classes=self.n_domains).type(torch.float)

        # mask out context if context_mask == 1
        context_mask = context_mask[:, None]
        context_mask_domain = context_mask.clone()
        context_mask = context_mask.repeat(1,self.n_classes)
        context_mask = (-1*(1-context_mask)) # need to flip 0 <-> 1
        c = c * context_mask

        context_mask_domain = context_mask_domain.repeat(1,self.n_domains)
        context_mask_domain = (-1*(1-context_mask_domain)) # need to flip 0 <-> 1
        domain = domain * context_mask_domain
        ...
        cemb_domain1 = self.contextembed_domain1(domain).view(-1, self.n_feat * 2, 1, 1)
        cemb_domain2 = self.contextembed_domain2(domain).view(-1, self.n_feat, 1, 1)
        cemb1 += cemb_domain1
        cemb2 += cemb_domain2
...
class DDPM(nn.Module):
    ...
    def forward_without_loss(self, x, domain: int):
        """
        this method is used in training, so samples t and noise randomly
        """

        _ts = torch.randint(1, self.n_T+1, (x.shape[0],)).to(self.device).repeat_interleave(self.nn_model.n_classes)  # t ~ Uniform(0, n_T)
        noise = torch.randn_like(x).repeat_interleave(self.nn_model.n_classes, axis=0)  # eps ~ N(0, 1)
        c = torch.arange(self.nn_model.n_classes, dtype=int).repeat(len(x)).to(noise.device)
        domain = torch.ones(len(c), dtype=int).to(noise.device) * domain
        x = x.repeat_interleave(self.nn_model.n_classes, axis=0)
        #import pdb
        #pdb.set_trace()
        x_t = (
    ...
    def forward(self, x, c, domain):
        ...
        context_mask = torch.bernoulli(torch.zeros_like(c)+self.drop_prob).to(self.device)
        # return MSE between added noise, and our predicted noise
        model_output = self.nn_model(x_t, c, domain, _ts / self.n_T, context_mask)
        batch_wise_mse = ((noise - model_output)**2).mean(dim=(1,2,3))
        return batch_wise_mse, _ts, noise

    def forward_neg(self, x, c, domain, _ts, noise):
        """
        this method is used in training, so samples t and noise randomly
        """
        old_c = c.clone()
        permute = np.random.choice(range(1, 9))
        c[c != 10] = (c[c != 10] + permute)%10
    ...
    def sample(self, n_sample, size, device, guide_w = 0.0):
        ...
        c_i = torch.stack([torch.tensor([i,j]) for j in torch.arange(0,4) for i in torch.arange(0,self.nn_model.n_classes) ]).to(device)
...
import os
dataset_names = ["MNIST","MNIST_M","SVHN","SYN_DIGITS"]
class MNIST_like(MNIST):
    def __init__(self, root, dataset, train = True, transform = None, target_transform = None, download = False, target_dataset=False):
        self.dataset = dataset
        super().__init__(root, train, transform, target_transform, download)
        assert(dataset in dataset_names)
        self.dataset_index = [index for index in range(len(dataset_names)) if self.dataset== dataset_names[index]][0]
        self.target_dataset = target_dataset
        
    def _check_exists(self):
        return os.path.exists(os.path.join(self.root, f"{self.dataset.lower()}/{self.dataset}_train.pt"))
        
    def _load_data(self):
        data, labels = torch.load(os.path.join(self.root, f"{self.dataset.lower()}/{self.dataset}_train.pt")) if self.train else torch.load(os.path.join(self.root, f"{self.dataset.lower()}/{self.dataset}_test.pt"))
...
def train_mnist():
    ...
    #dataset = torch.utils.data.Subset(dataset, np.random.choice(range(len(dataset)), size=4000, replace=False))
    dataloader = DataLoader(dataset, batch_size=batch_size*8, shuffle=True, num_workers=5)
    x_real_samples, c_real_samples, d_real_samples = next(iter(dataloader))
    ...
    from train import test_acc, domain_test_numpy
    ...
    for data in test_loader:
        check_images, check_labels = data
        check_images, check_labels = check_images.to(device), check_labels.to(device)
        break
    ...
    for ep in range(n_epoch):
        ...
        with torch.no_grad():
            ...
            for sample in range(100):
                import pdb
                pdb.set_trace()
                out = ddpm(check_images, check_labels, torch.ones(len(check_images), dtype=int).to(device) * 0)
                print(out)
        continue
        """
        with torch.no_grad():
